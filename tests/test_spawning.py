    ...
    for r in range(rows):
        for c in range(cols):
            if r < grid.rows and c < grid.cols:
                name = names[r][c]
                symbol = (
                    config.SYMBOLS.get(name) if name else config.SYMBOLS["EMPTY"]
                )  # Use EMPTY not None
...
class TestWildSpawning(unittest.TestCase):
    ...
    def test_one_wild_per_cluster_spawn(self):
        ...
        grid.landed_coords = set()  # Ensure no EWs are considered landed
        (
            _cleared,
            _ew_coll,
            _did_exp,
            _details,
            spawned_coords,
        ) = grid.process_explosions_and_spawns(clusters, set())

        self.assertEqual(len(spawned_coords), 3)
        for r, c in spawned_coords:
            spawned_symbol = grid._get_symbol(r, c)
            self.assertIsNotNone(
                spawned_symbol, f"Symbol at {r},{c} should not be None after spawn"
            )
            self.assertTrue(
    ...
    def test_spawn_location_in_footprint(self):
        ...
        with patch("simulator.config.SYMBOLS", self.DUMMY_SPAWN_SYMBOLS):
            ...
            original_footprint = set(coords_blu)
            self.assertEqual(
                original_footprint, {(1, 1), (1, 2), (2, 1), (2, 2), (3, 1)}
            )
    ...
    def test_spawn_collision_handling(self):
        ...
        with patch("simulator.config.SYMBOLS", self.DUMMY_SPAWN_SYMBOLS):
            ...
            self.assertTrue(
                all(
                    t in [SymbolType.WILD, SymbolType.EXPLOSIVO_WILD]
            ...
            self.assertTrue(
                all(coord in combined_footprint for coord in spawned_coords)
            )
