...
def setUpModule():
    ...
    if "TEST_RUN_SLOW_TESTS" not in os.environ:
        raise unittest.SkipTest(
            "Skipping slow integration tests. Set TEST_RUN_SLOW_TESTS=1 env var to run them."
        )
...
class TestLogFormatting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._original_timeout = (
            run_simulation.__defaults__[0]
            if hasattr(run_simulation, "__defaults__") and run_simulation.__defaults__
            else None
        )
    ...
    def test_csv_log_format(self):
        ...
        try:
            ...
            print(
                f"Debug: Files in temp dir before simulation: {os.listdir(self.temp_path)}"
            )
            ...
            # Print debug info
            print(
                f"Debug: Files in temp dir after simulation: {os.listdir(self.temp_path)}"
            )

            # Verify the CSV file was created
            self.assertTrue(
                os.path.exists(csv_path), f"CSV file {csv_path} was not created"
            )
            ...
            with open(csv_path, "r") as f:
                ...
                self.assertGreaterEqual(
                    len(rows), 1, "CSV should have at least a header row"
                )
                ...
                for column in expected_columns:
                    if column not in header:
                        print(
                            f"Error: Expected column '{column}' not found in header: {header}"
                        )

                self.assertIn(
                    "SpinNumber", header, "CSV should have a SpinNumber column"
                )
                self.assertIn("TotalWin", header, "CSV should have a TotalWin column")
                self.assertIn(
                    "TotalScattersInSequence",
                    header,
                    "CSV should have a TotalScattersInSequence column",
                )
                ...
                if len(rows) > 1:
                    ...
                    for i, row in enumerate(rows[1:], 1):
                        if len(row) != len(header):
                            print(
                                f"Error: Row {i} has {len(row)} columns, expected {len(header)}: {row}"
                            )
                        self.assertEqual(
                            len(row),
                            len(header),
    ...
    def test_summary_log_format(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                f"Summary file {summary_path} was not created",
            )
            ...
            with open(summary_path, "r") as f:
                ...
                print(
                    f"Debug: First 200 chars of summary content: {summary_content[:200]}"
                )
            ...
            for expected in expected_strings:
                ...
                self.assertIn(
                    expected,
                    summary_content,
                    f"Summary file should contain '{expected}'",
                )
                ...
                self.assertIn(
                    expected, output, f"Console output should contain '{expected}'"
                )

            # Win Distribution may or may not be present depending on implementation
            if (
                "Win Distribution" not in summary_content
                and "Win Distribution" not in output
            ):
                print(
                    "Warning: 'Win Distribution' not found in either summary or stdout"
                )
        except Exception as e:
            print(f"Debug: Encountered exception: {type(e).__name__}: {str(e)}")
            raise
