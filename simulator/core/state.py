...
class GameState:
    ...
    def initialize_spin(self):
        ...
        # Reset multiplier based on mode (BG or FS)
        if self.is_free_spins:
            trail = config.FS_MULTIPLIER_TRAILS.get(
                self.fs_base_multiplier, config.FS_MULTIPLIER_TRAILS[1]
            )
        else:
            trail = config.AVALANCHE_MULTIPLIERS_BG
        self.current_multiplier = trail[0]
        ...
        if self.fs_pending_upgrades > 0:
            for _ in range(self.fs_pending_upgrades):
                ...
                current_level_idx = config.FS_BASE_MULTIPLIER_LEVELS.index(
                    self.fs_base_multiplier
                )
                next_level_idx = min(
                    current_level_idx + 1, len(config.FS_BASE_MULTIPLIER_LEVELS) - 1
                )
                self.fs_base_multiplier = config.FS_BASE_MULTIPLIER_LEVELS[
                    next_level_idx
                ]
                # Add extra spin
    ...
    def update_after_clusters(self, clusters: List, did_explode: bool):
        ...
        if clusters:  # Increment only if there were winning clusters
            ...
            # Advance along the appropriate multiplier trail
            if self.is_free_spins:
                trail = config.FS_MULTIPLIER_TRAILS.get(
                    self.fs_base_multiplier, config.FS_MULTIPLIER_TRAILS[1]
                )
                ...
                trail = config.AVALANCHE_MULTIPLIERS_BG
            safe_index = min(self.current_multiplier_index, len(trail) - 1)
            self.current_multiplier = trail[safe_index]
        ...
        if not self.is_free_spins:  # Check for Base Game trigger
            if num_scatters >= 3:
                ...
                if spins_won is None:  # Handle 5+
                    ...
                    spins_won = (
                        config.FS_TRIGGER_SCATTERS[max_defined]
                        + (num_scatters - max_defined)
            ...
            if num_scatters >= 2:
                ...
                if spins_won is None:  # Handle 5+
                    ...
                    spins_won = (
                        config.FS_RETRIGGER_SCATTERS[max_defined]
                        + (num_scatters - max_defined)
            ...
            upgrades_earned = (
                self.fs_ew_collected_this_spin // config.FS_EW_COLLECTION_PER_UPGRADE
            )
    ...
    def consume_free_spin(self):
        ...
        if self.is_free_spins:
            ...
            # Ensure remaining spins do not go negative and end FS mode when zero
            if self.remaining_spins <= 0:
                self.is_free_spins = False  # End FS mode
                self.remaining_spins = 0
            assert (
                self.remaining_spins >= 0
            ), f"remaining_spins negative: {self.remaining_spins}"
